...
- De gebruiker werkt hoofdzakelijk op **Windows**, maar het script is tevens geschikt voor **Linux** (dual OS), specifiek voor installatie op de server **fabrix** (`192.168.178.40`) in de directory `scripts/`. De server is bereikbaar via SSH/SCP met de gebruiker `maarten` (de SSH-sleutels zijn geconfigureerd op de Windows-machine van de gebruiker).
- Het script op de server `fabrix` is een symlink van `~/html-album/html-album.py` naar `~/scripts/html-album.py`. updates worden doorgevoerd via Git deployment flow: `PC => GIT => fabrix` (door `git push fabrix master` of `git pull` in `/home/maarten/html-album/` op de server).
...
- Werk bij elke aanpassing (wijziging aan de code) de constante `VERSION` in `html-album.py` bij met de huidige datum en tijd in het formaat: `v2 (DD-MM-YYYY HH:MM)`.
