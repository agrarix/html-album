...
Standaardwaarde:
- Op Windows: `Generated by html-album v2.0 (DD-MM-YYYY HH:MM) (Windows)`
- Op Linux: `Generated by html-album v2.0 (DD-MM-YYYY HH:MM) (Linux) at hostname`

De volgende variabelen worden automatisch vervangen:
- `${PGM}` (of `{PGM}`): Programmanaam (`html-album`)
- `${VER}` (of `{VER}` / `{VERSION}`): Versie en buildtijd (bijv. `2.0 (07-07-2026 09:14)`)
