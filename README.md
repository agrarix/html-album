...
| `ICON` | Bestandsnaam van het albumicoon / favicon (wordt uitgesloten van de galerij) | `"Agrarix-Pingu_2017.jpg"` |
...
| `FOOTER` | Sjabloon voor de voettekst (optioneel, ondersteunt `${PGM}`, `${VER}`, `${DATE}`, `${TIME}`, `${OS}`, `${HOSTNAME}`). Standaard is dit dynamisch op basis van het OS. | `"Generated by ${PGM} v${VER} (${OS})"` (Windows) of `"Generated by ${PGM} v${VER} (${OS}) at ${HOSTNAME}"` (Linux) |
...
Standaardwaarde:
- Op Windows: `Generated by html-album v(DD-MM-YYYY HH:MM) (Windows)`
- Op Linux: `Generated by html-album v(DD-MM-YYYY HH:MM) (Linux) at hostname`

De volgende variabelen worden automatisch vervangen:
- `${PGM}` (of `{PGM}`): Programmanaam (`html-album`)
- `${VER}` (of `{VER}` / `{VERSION}`): Buildtijd (bijv. `(07-07-2026 09:16)`)
